...
```toml
[dependencies]
iced = { version = "0.13", features = ["wgpu", "canvas", "advanced", "svg", "image", "qr_code", "markdown", "highlighter"] }
```
...
*   `default(default: impl Into<T>) -> Self`
*   `on_release(message: Message) -> Self`
*   `step(step: T) -> Self`
*   `shift_step(shift_step: impl Into<T>) -> Self`
*   `width(width: impl Into<Length>) -> Self`
...
*   `content.cursor_position() -> (usize, usize)` - (line, column)
...
Subscriptions allow your application to listen to external events continuously (keyboard, mouse, window resizing, time).
